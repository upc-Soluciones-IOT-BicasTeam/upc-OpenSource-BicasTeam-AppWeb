...
export class ProfileEditionComponent implements OnInit {
  ...
  authenticateUser() {
    if (this.authEmail && this.authPassword) {
      ...
      this.iamApiService.authenticateUser(this.authEmail, this.authPassword).subscribe(
        (data: UserEntity) => {
          this.user = data;
  ...
  updateUser() {
    ...
    for (const key in this.user) {
      if (this.user.hasOwnProperty(key)) {
        ...
        if (this.user[userKey] !== undefined && this.user[userKey] !== this.originalUser[userKey]) {
          hasChanges = true;
          break;
        }
      }
    ...
    this.iamApiService.updateUser(this.authEmail, this.authPassword, this.user).subscribe(
      (data: UserEntity) => {
